...
var ready = function () {
    ...
    chatBox = {
        ...
        createChatBox: function (conversation_id, minimizeChatBox) {
           ...
           /* $.get("http://localhost:3003/conversations/" + conversation_id, function (data) {
            ...
            $.ajax({
                ...
                type: "GET",
                url: "http://localhost:3003/conversations/"+ conversation_id,
            ...
            if (event.keyCode == 13 && event.shiftKey == 0) {
                ...
                if (message != '') {
                        ...
                        $.ajax({
                            //url: "http://chattyboy.herokuapp.com/conversations",
                            type: "POST",
                            url: "http://localhost:3003/conversations/"+ conversation_id + "/messages",
                            crossDomain: true,
                            // Tell YQL what we want and that we want JSON
                            data: {
                                body: message,
                            ...
                            success: function( data ) {
                                //console.log(data);
                                $.post("/conversations/"+conversation_id+"/messages", { body: data.body , conversation_id: data.conversation_id , created_at: data.created_at , id: data.id , updated_at: data.updated_at, user_id: data.user_id }, function(data){
                                    console.log(data);
                                });
                                $(chatboxtextarea).val('');
